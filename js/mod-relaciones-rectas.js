...
registerModule({
  ...
  build(sec){
    ...
    function pintarC1(P){
      ...
      P.vector(O[0],O[1],O[0]+vG1[0],O[1]+vG1[1],{color:'--s1',grosor:2.4,etiqueta:'v'});
      P.vector(O[0],O[1],O[0]+nG1[0],O[1]+nG1[1],{color:'--s6',grosor:2.4,etiqueta:'n'});
      const magV=Math.hypot(vG1[0],vG1[1]), magN=Math.hypot(nG1[0],nG1[1]);
      const uV=[vG1[0]/magV,vG1[1]/magV], uN=[nG1[0]/magN,nG1[1]/magN];
      marcaRecta(P,O,uV,uN,0.35,{color:'--s4',grosor:2});
    ...
    function actualizarC1(){
      ...
      leerC1.textContent='v = (2, 3)  ·  n = (3, −2)  ·  v · n = '+dot.toFixed(2)+' (perpendiculares en cualquier punto de la recta)';
    }
    c1.append(el('div',{class:'controls'},
      el('label',{},'punto sobre la recta:'),
      el('input',{type:'range',min:'-1',max:'1',step:'0.05',value:String(sG1),
        oninput:e=>{ sG1=parseFloat(e.target.value); actualizarC1(); }})
    ));
    c1.append(leerC1);
    actualizarC1();
    ...
    c2.append(el('p',{},'La distancia de un punto a una recta es la del segmento perpendicular que los une: cualquier otro segmento hacia la recta es más largo. Un punto $P$ recorre una órbita alrededor de la recta y el pie de la perpendicular lo sigue; un segundo punto $Q$, controlado por el siguiente deslizador, se puede ubicar en cualquier lugar de la recta para comparar su distancia a $P$ con la distancia perpendicular.'));
    ...
    P2.animar((P,t)=>{
      ...
      leerC2.textContent='d(P, recta) = '+dPerp.toFixed(2)+'  ·  d(P, Q) = '+dQ.toFixed(2)
        +(igual?'  (Q coincide con el pie)':'  (más largo que la perpendicular)');
    },{duracion:10});
    c2.append(leerC2);
    c2.append(el('div',{class:'controls'},
      el('label',{},'Q sobre la recta:'),
      el('input',{type:'range',min:'-1',max:'3',step:'0.1',value:String(qG2),
        oninput:e=>{ qG2=parseFloat(e.target.value); }})
    ));
    ...
    P3.animar((P,t)=>{
      ...
      if(angDeg>1.5){
        ...
        estado='Secantes: se cruzan en un punto.';
      } else if(g>0.06){
        estado='Paralelas: mismo director, no se tocan.';
      } else {
        estado='Coincidentes: son la misma recta.';
      }
      leerC3.textContent=estado+'  (ángulo entre directores ≈ '+angDeg.toFixed(1)+'°, desplazamiento ≈ '+g.toFixed(2)+')';
    },{duracion:10});
    c3.append(leerC3);
    ...
    c4.append(el('p',{},'Dos rectas con pendientes $m_1$ y $m_2$ forman, en su punto de cruce, dos pares de ángulos opuestos: uno agudo y uno obtuso — o los cuatro rectos, si son perpendiculares. Por convención se reporta el agudo, y por eso la fórmula lleva valor absoluto. Los siguientes deslizadores cambian $m_1$ y $m_2$; el arco marca el ángulo agudo medido directamente en el dibujo, y el número de al lado lo compara con lo que da la fórmula.'));
    ...
    function actualizarC4(){
      ...
      if(Math.abs(denom)<0.03){
        leerC4.textContent='m1 = '+m1G4.toFixed(2)+'  ·  m2 = '+m2G4.toFixed(2)
          +'  ·  1 + m1·m2 ≈ 0 → la fórmula se indefine, pero el ángulo medido es 90° (perpendiculares)';
      } else {
        const tanTheta=Math.abs((m2G4-m1G4)/denom);
        const formulaDeg=Math.atan(tanTheta)*180/Math.PI;
        leerC4.textContent='m1 = '+m1G4.toFixed(2)+'  ·  m2 = '+m2G4.toFixed(2)
          +'  ·  ángulo medido en el dibujo ≈ '+medidoDeg.toFixed(1)+'°'
          +'  ·  fórmula: arctan|('+m2G4.toFixed(2)+'−'+m1G4.toFixed(2)+')/(1+'+m1G4.toFixed(2)+'·'+m2G4.toFixed(2)+')| ≈ '+formulaDeg.toFixed(1)+'°';
      }
    }
    c4.append(el('div',{class:'controls'},
      el('label',{},'m1:'),
      el('input',{type:'range',min:'-4',max:'4',step:'0.1',value:String(m1G4),
        oninput:e=>{ m1G4=parseFloat(e.target.value); actualizarC4(); }}),
      el('label',{},'m2:'),
      el('input',{type:'range',min:'-4',max:'4',step:'0.1',value:String(m2G4),
        oninput:e=>{ m2G4=parseFloat(e.target.value); actualizarC4(); }})
    ));
    c4.append(leerC4);
    actualizarC4();
